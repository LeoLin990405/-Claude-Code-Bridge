...
Automatically saves session context to Markdown files when /clear or /compact
...
class ContextSaver:
    """System 1: Instant context saver that archives sessions to Markdown."""
    ...
    def save_session(self, session_path: Path, force: bool = False) -> Optional[Path]:
        """
        Save a session as a Markdown archive.
        ...
        Returns:
            Path to the saved archive file, or None if skipped
        """
        ...
        md_content = self._generate_markdown(session)

        # Create output filename
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M")
        session_id = session.session_id[:8]
        output_path = self.archive_dir / f"{timestamp}_{session_id}.md"

        # Handle filename collision
        counter = 1
        while output_path.exists():
            output_path = self.archive_dir / f"{timestamp}_{session_id}_{counter}.md"
            counter += 1
