<p align="center">
  <img src="https://img.shields.io/badge/CCB-多AI协作平台-blue?style=for-the-badge" alt="CCB">
  <img src="https://img.shields.io/badge/Provider-9个-green?style=for-the-badge" alt="Providers">
  <img src="https://img.shields.io/badge/Agent-9个-orange?style=for-the-badge" alt="Agents">
</p>

<h1 align="center">CCB - Claude Code Bridge</h1>

<p align="center">
  <strong>企业级多 AI 编排平台</strong>
  <br>
  <em>智能路由、专业化 Agent、统一 API，支持 9 个 AI Provider</em>
</p>

<p align="center">
  <a href="#-特性">特性</a> •
  <a href="#-快速开始">快速开始</a> •
  <a href="#-架构">架构</a> •
  <a href="#-gateway-api">Gateway API</a> •
  <a href="#-agent-系统">Agent</a> •
  <a href="#-安装">安装</a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Python-3.9+-3776AB?logo=python&logoColor=white" alt="Python">
  <img src="https://img.shields.io/badge/FastAPI-009688?logo=fastapi&logoColor=white" alt="FastAPI">
  <img src="https://img.shields.io/badge/SQLite-003B57?logo=sqlite&logoColor=white" alt="SQLite">
  <img src="https://img.shields.io/badge/WebSocket-010101?logo=socket.io&logoColor=white" alt="WebSocket">
  <img src="https://img.shields.io/badge/License-MIT-yellow" alt="License">
</p>

[English](README.md) | **中文**

---

## 🎯 概述

**CCB (Claude Code Bridge)** 是一个生产级多 AI 编排平台，将 9 个 AI Provider 统一在单一智能接口下。它具备自动任务路由、针对不同领域的专业化 Agent，以及支持 WebSocket 的现代 Gateway API。

### 为什么选择 CCB？

| 挑战 | CCB 解决方案 |
|------|-------------|
| 多个 AI CLI 接口不统一 | **统一命令接口**，所有 Provider 一致 |
| 手动选择 Provider | **智能路由**，基于任务分析自动选择 |
| 无持久化或状态管理 | **SQLite 状态存储**，支持请求队列 |
| 依赖终端通信 | **REST API + WebSocket**，解耦架构 |
| 单 Provider 局限性 | **9 个专业化 Agent**，自动降级 |

---

## ✨ 特性

### 核心平台

| 特性 | 描述 |
|------|------|
| **9 个 AI Provider** | Claude、Codex、Gemini、OpenCode、DeepSeek、Droid、iFlow、Kimi、Qwen |
| **智能路由** | 基于关键词和文件模式的任务感知 Provider 选择 |
| **魔法关键词** | `@deep`、`@review`、`@all`、`@docs`、`@search` 触发特殊行为 |
| **统一 CLI** | 所有 Provider 使用一致的 `*ask` / `*ping` 命令 |

### Gateway API（Phase 5）

| 特性 | 描述 |
|------|------|
| **REST API** | `POST /api/ask`、`GET /api/reply/{id}`、`GET /api/status` |
| **WebSocket** | 实时请求/响应流，端点 `/api/ws` |
| **优先级队列** | 基于 SQLite 持久化的请求优先级 |
| **多后端支持** | HTTP API、CLI 执行、终端集成 |
| **健康监控** | 自动 Provider 健康检查和指标 |

### 高级能力

| 特性 | 描述 |
|------|------|
| **速率限制** | 每个 Provider 独立的令牌桶算法 |
| **MCP 聚合** | 跨 MCP 服务器的统一工具发现 |
| **OAuth2 认证** | 安全的 Web API 访问 |
| **LSP/AST 工具** | 基于 tree-sitter 的代码智能 |
| **批量处理** | 并行任务执行 |
| **智能缓存** | 减少重复 API 调用 |

---

## 🚀 快速开始

### 基本用法

```bash
# 智能路由 - 自动选择最佳 Provider
ccb ask "添加 React 组件"        # → Gemini (前端)
ccb ask "设计 API 接口"          # → Codex (后端)
ccb ask "分析算法复杂度"         # → DeepSeek (推理)

# 魔法关键词
ccb ask "@deep 分析这个算法"     # 强制深度推理
ccb ask "@review 检查这段代码"   # 强制代码审查
ccb ask "@all 最佳方案是什么"    # 多 Provider 查询

# 直接 Provider 命令
cask "你的问题"    # Codex
gask "你的问题"    # Gemini
dskask "你的问题"  # DeepSeek
kask "你的问题"    # Kimi
qask "你的问题"    # Qwen
```

### Gateway API

```bash
# 启动 Gateway 服务器
ccb-gateway start

# 通过 REST API 发送请求
curl -X POST http://localhost:8765/api/ask \
  -H "Content-Type: application/json" \
  -d '{"provider": "gemini", "message": "你好"}'

# 获取响应
curl http://localhost:8765/api/reply/{request_id}

# 检查系统状态
curl http://localhost:8765/api/status
```

### Agent 执行

```bash
# 自动选择最佳 Agent
ccb-agent auto "实现排序函数"

# 使用指定 Agent 执行
ccb-agent execute reviewer "审计这段代码"
ccb-agent execute oracle "分析算法复杂度"

# 列出可用 Agent
ccb-agent list
```

---

## 🏗️ 架构

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           CCB 平台架构                                       │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │                         Agent 层 (9 个 Agent)                          │ │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐    │ │
│  │  │ Sisyphus │ │  Oracle  │ │Librarian │ │ Explorer │ │ Frontend │    │ │
│  │  │  (代码)  │ │  (推理)  │ │  (文档)  │ │  (搜索)  │ │  (UI/UX) │    │ │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────┘    │ │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐                 │ │
│  │  │ Reviewer │ │ Workflow │ │ Polyglot │ │Autonomous│                 │ │
│  │  │  (审查)  │ │ (自动化) │ │  (国际化)│ │ (长任务) │                 │ │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘                 │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                    │                                         │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │                      Gateway API 层 (Phase 5)                          │ │
│  │  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌─────────────┐     │ │
│  │  │  REST API   │ │  WebSocket  │ │  请求队列   │ │  状态存储   │     │ │
│  │  │ (FastAPI)   │ │  (实时)     │ │  (优先级)   │ │  (SQLite)   │     │ │
│  │  └─────────────┘ └─────────────┘ └─────────────┘ └─────────────┘     │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                    │                                         │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │                         统一路由引擎                                    │ │
│  │         任务分析 → Provider 选择 → 降级链                               │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                    │                                         │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │                           后端层                                        │ │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐                 │ │
│  │  │ HTTP API │ │ CLI 执行 │ │   终端   │ │   FIFO   │                 │ │
│  │  │(Anthropic│ │ (Codex,  │ │  (兼容)  │ │  (兼容)  │                 │ │
│  │  │ DeepSeek)│ │ Gemini)  │ │          │ │          │                 │ │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘                 │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                    │                                         │
│  ┌────────────────────────────────────────────────────────────────────────┐ │
│  │                      Provider 层 (9 个 Provider)                       │ │
│  │  ┌───────┐ ┌───────┐ ┌───────┐ ┌────────┐ ┌────────┐                 │ │
│  │  │Claude │ │ Codex │ │Gemini │ │OpenCode│ │DeepSeek│                 │ │
│  │  └───────┘ └───────┘ └───────┘ └────────┘ └────────┘                 │ │
│  │  ┌───────┐ ┌───────┐ ┌───────┐ ┌────────┐                            │ │
│  │  │ Droid │ │ iFlow │ │ Kimi  │ │  Qwen  │                            │ │
│  │  └───────┘ └───────┘ └───────┘ └────────┘                            │ │
│  └────────────────────────────────────────────────────────────────────────┘ │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 🌐 Gateway API

### REST 端点

| 方法 | 端点 | 描述 |
|------|------|------|
| `POST` | `/api/ask` | 向 Provider 提交请求 |
| `GET` | `/api/reply/{request_id}` | 获取请求的响应 |
| `GET` | `/api/status` | 获取 Gateway 和 Provider 状态 |
| `DELETE` | `/api/request/{request_id}` | 取消待处理请求 |
| `GET` | `/docs` | 交互式 API 文档 |

### 请求示例

```bash
# 提交请求
curl -X POST http://localhost:8765/api/ask \
  -H "Content-Type: application/json" \
  -d '{
    "provider": "gemini",
    "message": "解释 Python 中的 async/await",
    "timeout_s": 60,
    "priority": 50
  }'

# 响应
{
  "request_id": "abc123-def",
  "provider": "gemini",
  "status": "queued"
}
```

### WebSocket 事件

```javascript
// 连接 WebSocket
const ws = new WebSocket('ws://localhost:8765/api/ws');

// 订阅事件
ws.send(JSON.stringify({
  type: 'subscribe',
  channels: ['requests', 'providers']
}));

// 接收事件
ws.onmessage = (event) => {
  const data = JSON.parse(event.data);
  // { type: 'request_update', request_id: '...', status: 'completed', response: '...' }
};
```

### Provider 状态

```bash
curl http://localhost:8765/api/status | jq '.providers'
```

```json
[
  {"name": "gemini", "enabled": true, "status": "healthy", "avg_latency_ms": 2500},
  {"name": "codex", "enabled": true, "status": "healthy", "avg_latency_ms": 5800},
  {"name": "deepseek", "enabled": true, "status": "healthy", "avg_latency_ms": 48000},
  {"name": "kimi", "enabled": true, "status": "healthy", "avg_latency_ms": 5000},
  {"name": "qwen", "enabled": true, "status": "healthy", "avg_latency_ms": 11000},
  {"name": "iflow", "enabled": true, "status": "healthy", "avg_latency_ms": 40000},
  {"name": "opencode", "enabled": true, "status": "healthy", "avg_latency_ms": 23000}
]
```

---

## 🤖 Agent 系统

### Agent 概览

| Agent | 用途 | 首选 Provider | 关键词 |
|-------|------|---------------|--------|
| **Sisyphus** | 代码实现 | Codex, Gemini | 实现, 创建, 开发 |
| **Oracle** | 深度推理与分析 | DeepSeek, Claude | 分析, 推理, 算法 |
| **Librarian** | 文档与搜索 | Claude, Gemini | 文档, 解释 |
| **Explorer** | 代码库导��� | Gemini, Claude | 查找, 搜索, 定位 |
| **Frontend** | UI/UX 开发 | Gemini, Claude | react, vue, 组件 |
| **Reviewer** | 代码审查与测试 | Gemini, Claude | 审查, 审计, 检查 |
| **Workflow** | 多步骤自动化 | iFlow, Droid | 自动化, 流程 |
| **Polyglot** | 翻译与国际化 | Kimi, Qwen | 翻译, 多语言 |
| **Autonomous** | 长时间任务 | Droid, Codex | 后台, 长时间 |

### Agent 命令

```bash
# 列出所有 Agent 及其能力
ccb-agent list

# 根据任务自动选择 Agent
ccb-agent auto "实现二叉搜索树"

# 使用指定 Agent 执行
ccb-agent execute sisyphus "创建 REST API 端点"
ccb-agent execute oracle "分析这个算法的时间复杂度"
ccb-agent execute reviewer "审查这个 PR"

# 显示哪个 Agent 会被选中
ccb-agent match "将这份文档翻译成中文"
```

---

## 📦 Provider 列表

### Provider 矩阵

| Provider | 命令 | 后端 | 最佳用途 | 状态 |
|----------|------|------|----------|------|
| **Claude** | `lask` | HTTP API | 架构设计、通用 | ✅ |
| **Codex** | `cask` | CLI (`exec --json`) | 后端、API | ✅ |
| **Gemini** | `gask` | CLI (`-p`) | 前端、审查 | ✅ |
| **OpenCode** | `oask` | CLI (`run --format json`) | 通用编码 | ✅ |
| **DeepSeek** | `dskask` | CLI (`-q`) | 深度推理 | ✅ |
| **Droid** | `dask` | 终端 | 自主执行 | ⚠️ |
| **iFlow** | `iask` | CLI (`-p`) | 工作流 | ✅ |
| **Kimi** | `kask` | CLI (`--quiet -p`) | 中文、长上下文 | ✅ |
| **Qwen** | `qask` | CLI | 多语言 | ✅ |

### 路由规则

| 任务类型 | 关键词 | 文件模式 | Provider |
|----------|--------|----------|----------|
| 前端 | react, vue, 组件 | `*.tsx`, `*.vue` | Gemini |
| 后端 | api, 接口, 服务器 | `api/**`, `routes/**` | Codex |
| 推理 | 分析, 算法 | - | DeepSeek |
| 架构 | 设计, 架构 | - | Claude |
| 审查 | 审查, 检查, 审计 | - | Gemini |

### 魔法关键词

| 关键词 | 动作 | 描述 |
|--------|------|------|
| `@deep` | 深度推理 | 强制使用 DeepSeek |
| `@review` | 代码审查 | 强制 Gemini 审查模式 |
| `@docs` | 文档查询 | 查询 Context7 |
| `@search` | 网络搜索 | 触发网络搜索 |
| `@all` | 多 Provider | 同时查询多个 Provider |

---

## 📁 项目结构

```
~/.local/share/codex-dual/
├── bin/                        # CLI 命令
│   ├── ccb-ask                 # 智能路由命令
│   ├── ccb-agent               # Agent 执行
│   ├── ccb-gateway             # Gateway 管理
│   ├── ccb-ratelimit           # 速率限制
│   └── cask, gask, dskask...   # Provider 专用命令
│
├── lib/                        # 核心模块
│   ├── unified_router.py       # 智能路由引擎
│   ├── agent_registry.py       # Agent 定义
│   ├── agent_executor.py       # Agent 执行逻辑
│   ├── provider_commands.py    # Provider 命令映射
│   │
│   └── gateway/                # Gateway API 模块 (Phase 5)
│       ├── gateway_server.py   # FastAPI 服务器
│       ├── gateway_api.py      # REST 端点
│       ├── gateway_config.py   # 配置管理
│       ├── state_store.py      # SQLite 状态持久化
│       ├── request_queue.py    # 优先级队列
│       ├── monitor.py          # 实时监控
│       ├── models.py           # 数据模型
│       └── backends/           # 后端实现
│           ├── base_backend.py
│           ├── http_backend.py
│           ├── cli_backend.py
│           └── terminal_backend.py
│
├── mcp/                        # MCP 服务器
├── config/                     # 配置模板
│   └── gateway.yaml            # Gateway 配置
│
└── install.sh                  # 安装脚本

~/.ccb_config/                  # 用户配置
├── unified-router.yaml         # 路由规则
├── phase4.yaml                 # 高级特性
└── gateway.db                  # Gateway 状态数据库
```

---

## 🔧 安装

### 前置条件

- **Python 3.9+**
- **WezTerm** 或 **tmux**（终端复用）
- Provider CLI：`claude`、`codex`、`gemini`、`opencode`、`deepseek`、`kimi`、`qwen`

### 安装步骤

```bash
# 克隆仓库
git clone https://github.com/LeoLin990405/ccb.git ~/.local/share/codex-dual

# 运行安装
cd ~/.local/share/codex-dual && ./install.sh

# 添加到 PATH（添加到 ~/.zshrc 或 ~/.bashrc）
export PATH="$HOME/.local/share/codex-dual/bin:$PATH"
```

### 环境变量

```bash
# 添加到 ~/.zshrc 或 ~/.bashrc
export CCB_SIDECAR_AUTOSTART=1      # 自动启动 sidecar pane
export CCB_SIDECAR_DIRECTION=right  # Sidecar pane 方向
export CCB_CLI_READY_WAIT_S=20      # CLI 就绪超时
export CCB_USE_GATEWAY=1            # 启用 Gateway 模式
export CCB_GATEWAY_PORT=8765        # Gateway 端口
```

### 验证安装

```bash
# 检查 CCB 版本
ccb --version

# 检查 Provider 健康状态
ccb health

# 启动 Gateway
ccb-gateway start

# 测试路由
ccb route "实现一个 React 组件"
```

---

## 📊 性能指标

### Gateway 指标

| 指标 | 描述 |
|------|------|
| `total_requests` | 已处理请求总数 |
| `active_requests` | 当前处理中 |
| `queue_depth` | 待处理请求 |
| `avg_latency_ms` | 平均响应时间 |
| `success_rate` | 请求成功率 |

### Provider 延迟（典型值）

| Provider | 平均延迟 | 用途 |
|----------|----------|------|
| Gemini | ~2.5s | 快速响应 |
| Codex | ~5.8s | 代码生成 |
| Kimi | ~5.0s | 中文内容 |
| Qwen | ~11s | 多语言 |
| OpenCode | ~23s | 通用编码 |
| iFlow | ~40s | 工作流自动化 |
| DeepSeek | ~48s | 深度推理 |

---

## 🙏 致谢

本项目站在巨人的肩膀上：

- **[bfly123/claude_code_bridge](https://github.com/bfly123/claude_code_bridge)** - 原始多 AI 协作框架
- **[Grafbase/Nexus](https://github.com/grafbase/nexus)** - AI 网关架构灵感
- **[code-yeongyu/oh-my-opencode](https://github.com/code-yeongyu/oh-my-opencode)** - Agent 编排模式

---

## 👥 贡献者

- **Leo** ([@LeoLin990405](https://github.com/LeoLin990405)) - 项目负责人
- **Claude** (Anthropic Claude Opus 4.5) - 架构设计与实现

---

## 📄 许可证

MIT 许可证 - 详见 [LICENSE](LICENSE)

---

<p align="center">
  <sub>人与 AI 协作构建</sub>
  <br>
  <sub>⭐ 如果觉得有用，请给个 Star！</sub>
</p>
