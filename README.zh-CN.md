<p align="center">
  <img src="https://img.shields.io/github/stars/LeoLin990405/ai-router-ccb?style=social" alt="Stars">
  <img src="https://img.shields.io/github/license/LeoLin990405/ai-router-ccb?color=blue" alt="License">
  <img src="https://img.shields.io/badge/Python-3.9+-3776AB?logo=python&logoColor=white" alt="Python">
  <img src="https://img.shields.io/badge/FastAPI-009688?logo=fastapi&logoColor=white" alt="FastAPI">
</p>

<h1 align="center">
  <br>
  🤖
  <br>
  CCB Gateway
  <br>
</h1>

<h4 align="center">企业级多 AI 编排平台</h4>

<p align="center">
  <em>Claude 作为主脑，通过统一 Gateway API 调度 7 个 AI Provider，支持实时监控</em>
</p>

<p align="center">
  <a href="#-特性">特性</a> •
  <a href="#-快速开始">快速开始</a> •
  <a href="#-web-ui">Web UI</a> •
  <a href="#-api-参考">API</a> •
  <a href="#-架构">架构</a> •
  <a href="#-安装">安装</a>
</p>

<p align="center">
  <a href="README.md">English</a> | <strong>简体中文</strong>
</p>

<p align="center">
  <img src="screenshots/dashboard.png" alt="CCB Gateway 仪表盘" width="800">
</p>

---

## 概述

**CCB Gateway** 是一个生产级多 AI 编排平台，**Claude 作为主脑（Orchestrator）**，通过统一的 Gateway API 智能调度 7 个 AI Provider。

```
                    ┌─────────────────────────────┐
                    │     Claude (主脑)           │
                    │      Claude Code CLI        │
                    └─────────────┬───────────────┘
                                  │
                    ┌─────────────▼───────────────┐
                    │      CCB Gateway API        │
                    │    http://localhost:8765    │
                    └─────────────┬───────────────┘
                                  │
          ┌───────────┬───────────┼───────────┬───────────┐
          ▼           ▼           ▼           ▼           ▼
     ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐
     │  Kimi   │ │  Qwen   │ │DeepSeek │ │  Codex  │ │ Gemini  │
     │  🚀 7s  │ │  🚀 12s │ │  ⚡ 16s │ │ 🐢 48s  │ │ 🐢 71s  │
     └─────────┘ └─────────┘ └─────────┘ └─────────┘ └─────────┘
                      ┌─────────┐ ┌─────────┐
                      │  iFlow  │ │OpenCode │
                      │  ⚡ 25s │ │  ⚡ 42s │
                      └─────────┘ └─────────┘
```

### 为什么选择 CCB Gateway？

| 挑战 | 解决方案 |
|------|----------|
| 多个 AI CLI 接口不统一 | **统一 Gateway API**，所有 Provider 一致 |
| 手动选择 Provider | **智能路由**，基于速度分级自动降级 |
| 无法观察 AI 操作 | **实时监控**，WebSocket + Web UI |
| 无缓存或重试逻辑 | **内置缓存、重试和降级链** |
| 看不到 AI 思考过程 | **思考链 & 原始输出捕获** |
| Gemini OAuth 令牌过期 | **自动刷新令牌** - 无需手动重新认证 |

---

## ✨ 特性

### 核心网关

- **REST API** - `POST /api/ask`, `GET /api/reply/{id}`, `GET /api/status`
- **WebSocket** - 实时事件推送 `/api/ws`
- **优先级队列** - SQLite 持久化的请求优先级队列
- **多后端** - HTTP API、CLI 执行、WezTerm 集成
- **健康监控** - 自动 Provider 健康检查

### 生产级功能

- **API 认证** - 基于 API Key 的认证，SHA-256 哈希
- **限流** - 令牌桶算法，支持按 Key 限流
- **响应缓存** - SQLite 缓存，支持 TTL 和模式排除
- **重试与降级** - 指数退避，自动 Provider 降级
- **并行查询** - 同时查询多个 Provider
- **Prometheus 指标** - `/metrics` 端点用于监控
- **流式响应** - Server-Sent Events 实时响应

### CLI 监控

- **思考链捕获** - 从 `<thinking>` 标签、`[Thinking]` 块中提取推理过程
- **原始输出存储** - 完整 CLI 输出保存用于调试
- **JSONL 解析** - Codex/OpenCode 结构化输出提取
- **Web UI 展示** - 请求详情中可折叠的思考链和原始输出

### Provider 速度分级（新功能）

| 分级 | Providers | 响应时间 | 适用场景 |
|------|-----------|----------|----------|
| 🚀 **快速** | Kimi, Qwen | 5-15 秒 | 快速任务、简单问题 |
| ⚡ **中速** | DeepSeek, iFlow, OpenCode | 15-60 秒 | 复杂推理、编程 |
| 🐢 **慢速** | Codex, Gemini | 60-120 秒 | 深度分析、代码审查 |

### Gemini OAuth 自动刷新（新功能）

- **自动刷新令牌** - 无需手动重新认证
- **无缝集成** - 每次请求前自动检查并刷新过期令牌
- **Gemini 会员支持** - Google One AI Premium 会员免费使用

---

## 🚀 快速开始

### 启动网关

```bash
# 启动网关服务器
cd ~/.local/share/codex-dual
python3 -m lib.gateway.gateway_server --port 8765

# 或使用配置文件
python3 -m lib.gateway.gateway_server --config ~/.ccb_config/gateway.yaml
```

### 发送请求

```bash
# 向快速 Provider 提交请求
curl -X POST http://localhost:8765/api/ask \
  -H "Content-Type: application/json" \
  -d '{"provider": "kimi", "message": "你好"}'

# 获取响应
curl "http://localhost:8765/api/reply/{request_id}"

# 并行查询所有 Provider
curl -X POST http://localhost:8765/api/ask \
  -H "Content-Type: application/json" \
  -d '{"provider": "@all", "message": "2+2等于多少？", "aggregation_strategy": "first_success"}'

# 只查询快速 Provider
curl -X POST http://localhost:8765/api/ask \
  -H "Content-Type: application/json" \
  -d '{"provider": "@fast", "message": "快速问题"}'
```

### 检查状态

```bash
# 网关状态（包含 Provider 延迟）
curl http://localhost:8765/api/status

# Prometheus 指标
curl http://localhost:8765/metrics
```

---

## 🖥️ Web UI

启动网关后访问 `http://localhost:8765/` 打开 Web UI。

<p align="center">
  <img src="screenshots/dashboard.png" alt="仪表盘" width="800">
  <br>
  <em>仪表盘 - 实时网关统计和 Provider 状态</em>
</p>

### 仪表盘
- 实时网关统计和 Provider 状态
- 请求时间线可视化
- WebSocket 实时活动日志
- **Provider 速度指示器**（🚀/⚡/🐢）

### 请求管理
- 可配置页面大小的分页
- 按 Provider、状态、内容搜索和过滤
- 一键重试失败请求
- **查看每个请求的思考链和原始输出**

### 测试控制台
- 交互式 API 测试
- Provider 选择，支持自动路由
- 流式响应开关

### 对比模式
- 并排 Provider 对比
- 同时查询多个 Provider

### 功能特性
- **深色/浅色主题** - 切换并持久化到 localStorage
- **国际化支持** - 中英文双语
- **键盘快捷键** - `1-6` 切换标签页，`R` 刷新，`T` 测试，`?` 帮助
- **确认对话框** - 防止误删除
- **复制到剪贴板** - 一键复制 API Key

---

## 📡 API 参考

### 端点

| 方法 | 端点 | 描述 |
|------|------|------|
| `POST` | `/api/ask` | 向 Provider 提交请求 |
| `GET` | `/api/reply/{id}` | 获取响应 |
| `GET` | `/api/status` | 网关和 Provider 状态 |
| `GET` | `/api/requests` | 列出最近请求（支持分页） |
| `DELETE` | `/api/request/{id}` | 取消待处理请求 |
| `GET` | `/api/health` | 健康检查 |
| `GET` | `/api/cache/stats` | 缓存统计 |
| `DELETE` | `/api/cache` | 清除缓存 |
| `GET` | `/metrics` | Prometheus 指标 |
| `GET` | `/docs` | 交互式 API 文档 |

### 请求体

```json
{
  "provider": "kimi",
  "message": "你的问题",
  "timeout_s": 120,
  "priority": 50,
  "cache_bypass": false,
  "aggregation_strategy": null
}
```

### Provider 分组

| 分组 | Providers | 使用场景 |
|------|-----------|----------|
| `@all` | 全部 7 个 | 全面对比 |
| `@fast` | Kimi, Qwen | 快速响应 |
| `@chinese` | Kimi, Qwen, DeepSeek | 中文任务 |
| `@coding` | Codex, OpenCode, Kimi | 代码生成 |

### 响应（包含 thinking/raw_output）

```json
{
  "request_id": "abc123-def",
  "status": "completed",
  "response": "答案是...",
  "thinking": "<提取的思考链（如有）>",
  "raw_output": "<完整 CLI 输出用于调试>",
  "latency_ms": 1234.56,
  "cached": false
}
```

### API 认证

```bash
# 创建 API Key
curl -X POST http://localhost:8765/api/admin/keys \
  -H "Content-Type: application/json" \
  -d '{"name": "my-app", "rate_limit_rpm": 100}'

# 使用 API Key
curl -X POST http://localhost:8765/api/ask \
  -H "X-API-Key: your-api-key" \
  -H "Content-Type: application/json" \
  -d '{"provider": "kimi", "message": "你好"}'
```

---

## 🏗️ 架构

```
┌─────────────────────────────────────────────────────────────────────┐
│                      CCB Gateway 架构                                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │                 Claude (主脑 / Orchestrator)                   │ │
│  │                  智能任务调度与协调                             │ │
│  └────────────────────────────────────────────────────────────────┘ │
│                                  │                                   │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │                      Gateway API 层                            │ │
│  │  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐  │ │
│  │  │REST API │ │WebSocket│ │  认证   │ │  限流   │ │  指标   │  │ │
│  │  │(FastAPI)│ │ (事件)  │ │(API Key)│ │(令牌桶) │ │(Prometh)│  │ │
│  │  └─────────┘ └─────────┘ └─────────┘ └─────────┘ └─────────┘  │ │
│  └────────────────────────────────────────────────────────────────┘ │
│                                  │                                   │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │                        处理层                                  │ │
│  │  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐  │ │
│  │  │  缓存   │ │  重试   │ │  并行   │ │  流式   │ │ 思考链  │  │ │
│  │  │(SQLite) │ │ (降级)  │ │(多AI)   │ │ (SSE)   │ │  提取   │  │ │
│  │  └─────────┘ └─────────┘ └─────────┘ └─────────┘ └─────────┘  │ │
│  └────────────────────────────────────────────────────────────────┘ │
│                                  │                                   │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │              Provider 层 (7 个 Provider，3 个分级)             │ │
│  │                                                                 │ │
│  │  🚀 快速 (5-15s)    ⚡ 中速 (15-60s)    🐢 慢速 (60-120s)     │ │
│  │  ┌──────┐ ┌──────┐  ┌────────┐ ┌───────┐  ┌───────┐ ┌───────┐ │ │
│  │  │ Kimi │ │ Qwen │  │DeepSeek│ │ iFlow │  │ Codex │ │Gemini │ │ │
│  │  └──────┘ └──────┘  └────────┘ └───────┘  └───────┘ └───────┘ │ │
│  │                     ┌────────┐                                 │ │
│  │                     │OpenCode│                                 │ │
│  │                     └────────┘                                 │ │
│  └────────────────────────────────────────────────────────────────┘ │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### Provider 矩阵

| Provider | 分级 | 平均延迟 | 最佳用途 |
|----------|------|----------|----------|
| **Kimi** | 🚀 快速 | ~7s | 中文、长上下文 |
| **Qwen** | 🚀 快速 | ~12s | 多语言、通用 |
| **DeepSeek** | ⚡ 中速 | ~16s | 深度推理 |
| **iFlow** | ⚡ 中速 | ~25s | 工作流自动化 |
| **OpenCode** | ⚡ 中速 | ~42s | 通用编程 |
| **Codex** | 🐢 慢速 | ~48s | 代码生成 |
| **Gemini** | 🐢 慢速 | ~71s | 前端、代码审查 |

### 降级链

```yaml
fallback_chains:
  kimi: ["qwen", "deepseek"]
  qwen: ["kimi", "deepseek"]
  deepseek: ["kimi", "qwen"]
  gemini: ["kimi", "qwen"]
  codex: ["kimi", "qwen"]
```

> **注意**: Claude 是主脑，不参与任务调度。

---

## 📦 安装

### 前置条件

- **Python 3.9+**
- **WezTerm**（推荐）或 tmux
- 已安装 Provider CLI: `codex`, `gemini`, `opencode`, `deepseek`, `kimi`, `qwen`, `iflow`

### 安装步骤

```bash
# 克隆仓库
git clone https://github.com/LeoLin990405/ai-router-ccb.git ~/.local/share/codex-dual

# 安装依赖
pip install fastapi uvicorn pyyaml aiohttp prometheus-client

# 启动网关
cd ~/.local/share/codex-dual
python3 -m lib.gateway.gateway_server --port 8765

# 打开 Web UI
open http://localhost:8765/
```

### 配置

```yaml
# ~/.ccb_config/gateway.yaml
server:
  host: "127.0.0.1"
  port: 8765

default_provider: "kimi"  # 默认使用快速 Provider

providers:
  kimi:
    enabled: true
    backend_type: "cli_exec"
    cli_command: "kimi"
    cli_args: ["--quiet", "-p"]
    timeout_s: 120
    priority: 90  # 快速 Provider 高优先级

  gemini:
    enabled: true
    backend_type: "cli_exec"
    cli_command: "gemini"
    cli_args: ["-p"]  # 非交互式提示模式
    timeout_s: 120

retry:
  enabled: true
  max_retries: 2
  fallback_enabled: true
  fallback_chains:
    gemini: ["kimi", "qwen"]
```

### Gemini 设置（OAuth）

适用于 Google One AI Premium / Gemini 会员：

```bash
# 首次认证（只需一次）
gemini

# 内置自动刷新令牌 - 无需手动重新认证！
```

---

## 🔄 最近更新

### v0.9.x - Provider 优化（最新）
- **Provider 速度分级** - 快速/中速/慢速分类
- **Gemini OAuth 自动刷新** - 无缝令牌管理
- **改进降级链** - 基于速度的智能降级
- **Provider 分组** - `@fast`、`@chinese`、`@coding`
- **超时优化** - 更快的响应

### v0.8.x - CLI 监控
- **思考链捕获** - 提取并展示 AI 推理过程
- **原始输出存储** - 完整 CLI 输出保存到数据库
- **改进输出清理** - 更好的 Gemini 格式 JSON 提取
- **Web UI 增强** - 可折叠的思考链/原始输出展示

### v0.7.x - 生产级功能
- API 认证与限流
- 响应缓存与 TTL
- 重试与降级机制
- Prometheus 指标集成

---

## 🙏 致谢

- **[bfly123/claude_code_bridge](https://github.com/bfly123/claude_code_bridge)** - 原始多 AI 协作框架
- **[Grafbase/Nexus](https://github.com/grafbase/nexus)** - AI 网关架构灵感

---

## 👥 贡献者

- **Leo** ([@LeoLin990405](https://github.com/LeoLin990405)) - 项目负责人
- **Claude** (Anthropic Claude Opus 4.5) - 架构与实现

---

## 📄 许可证

MIT 许可证 - 详见 [LICENSE](LICENSE)

---

<p align="center">
  <sub>人机协作共同构建</sub>
  <br>
  <sub>⭐ 如果觉得有用，请给个 Star！</sub>
</p>
