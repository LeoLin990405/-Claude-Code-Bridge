# HiveMind WebUI Mode - Startup Guide

HiveMind supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts HiveMind with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use HiveMind from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\HiveMind\HiveMind.exe" --webui

# Or if HiveMind is in your PATH
HiveMind.exe --webui
```
...
```bash
# Using full path
/Applications/HiveMind.app/Contents/MacOS/HiveMind --webui

# Or using open command
open -a HiveMind --args --webui
```

### Method 2: Create Shell Script

Create `start-hivemind-webui.sh`:
...
```bash
chmod +x start-hivemind-webui.sh
./start-hivemind-webui.sh
```
...
chmod +x HiveMind-*.AppImage

# Run with --webui flag
./HiveMind-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/hivemind-webui.desktop`:

```ini
[Desktop Entry]
Name=HiveMind WebUI
Comment=Start HiveMind in WebUI mode
Exec=/opt/HiveMind/hivemind --webui
Icon=hivemind
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/hivemind-webui.desktop
```
...
Create `~/bin/start-hivemind-webui.sh`:
...
```bash
chmod +x ~/bin/start-hivemind-webui.sh
```
...
Create `/etc/systemd/system/hivemind-webui.service`:

```ini
[Unit]
Description=HiveMind WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/HiveMind/hivemind --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable hivemind-webui.service
sudo systemctl start hivemind-webui.service

# Check status
sudo systemctl status hivemind-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run HiveMind in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making HiveMind accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running HiveMind WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/iOfficeAI/HiveMind/issues/217)
...
#### 4. Download and Install HiveMind

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/iOfficeAI/HiveMind/releases
wget https://github.com/iOfficeAI/HiveMind/releases/download/vVERSION/HiveMind_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/iOfficeAI/HiveMind/releases/download/vVERSION/HiveMind_VERSION_arm64.deb

# Install the package
apt install -y ./HiveMind_*.deb

# Verify installation
which HiveMind
```

#### 5. Launch HiveMind WebUI

```bash
# Start HiveMind in WebUI mode with no-sandbox flag
HiveMind --no-sandbox --webui
```
...
To access HiveMind from other devices on your local network:

```bash
# Start with --remote flag
HiveMind --no-sandbox --webui --remote
...
HiveMind --no-sandbox --webui --port 8080
```
...
chmod +x /opt/HiveMind/hivemind
```

#### Out of Memory

HiveMind requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if HiveMind is running: look for "Server started" message
...
4. **Keep device charged** - Running HiveMind consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **HiveMind Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch HiveMind quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-hivemind.sh << 'EOF'
#!/bin/bash
echo "Starting HiveMind WebUI..."
HiveMind --no-sandbox --webui --remote
EOF

# Make executable
chmod +x ~/start-hivemind.sh
...
```bash
proot-distro login ubuntu -- bash -c "HiveMind --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/iOfficeAI/HiveMind/issues/217)
...
```cmd
HiveMind.exe --webui --remote
```

### macOS

```bash
/Applications/HiveMind.app/Contents/MacOS/HiveMind --webui --remote
```

### Linux

```bash
hivemind --webui --remote
```
...
netsh advfirewall firewall add rule name="HiveMind WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add HiveMind
...
which hivemind
# or
find /opt -name "hivemind" 2>/dev/null
```
...
```powershell
& "C:\Program Files\HiveMind\HiveMind.exe" --webui 2>&1 | Tee-Object -FilePath hivemind.log
```

**macOS/Linux:**

```bash
/path/to/hivemind --webui 2>&1 | tee hivemind.log
```
...
export HIVEMIND_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as HIVEMIND_ALLOW_REMOTE=true)
export HIVEMIND_HOST=0.0.0.0

# Then start the application
hivemind --webui

# You can also pass the port directly via CLI
hivemind --webui --port 8080
```
...
| Platform | Location                                                 |
| -------- | -------------------------------------------------------- |
| Windows  | `%APPDATA%/HiveMind/webui.config.json`                     |
| macOS    | `~/Library/Application Support/HiveMind/webui.config.json` |
| Linux    | `~/.config/HiveMind/webui.config.json`                     |
...
```cmd
# Using full path
"C:\Program Files\HiveMind\HiveMind.exe" --resetpass

# Or for a specific user
"C:\Program Files\HiveMind\HiveMind.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/HiveMind.app/Contents/MacOS/HiveMind --resetpass

# Or for a specific user
/Applications/HiveMind.app/Contents/MacOS/HiveMind --resetpass username
```
...
# Or for a specific user
hivemind --resetpass username

# Or using full path
/opt/HiveMind/hivemind --resetpass
```
...
- [GitHub Issues](https://github.com/iOfficeAI/HiveMind/issues)
...
2. Search [existing issues](https://github.com/iOfficeAI/HiveMind/issues)
3. Create a [new issue](https://github.com/iOfficeAI/HiveMind/issues/new) with:
   - Your OS and version
   - HiveMind version
   - Steps to reproduce
...
**Happy using HiveMind in WebUI mode!** 🚀
