...
export const CSRF_COOKIE_NAME = 'hivemind-csrf-token';
...
export const AUTH_CONFIG = {
  ...
  COOKIE: {
    ...
    NAME: 'hivemind-session' as const,
    OPTIONS: {
 ...
 * 建议生产环境配置 HTTPS 并设置 HIVEMIND_HTTPS=true
 * Recommend configuring HTTPS in production and setting HIVEMIND_HTTPS=true
 */
export function getCookieOptions(): {
  ...
  const isHttps = process.env.HIVEMIND_HTTPS === 'true' || (process.env.NODE_ENV === 'production' && process.env.HTTPS === 'true');
