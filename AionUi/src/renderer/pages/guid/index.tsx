  ...
  const handleSend = async () => {
    ...
    if (!selectedAgent || selectedAgent === 'gemini' || (isPreset && finalEffectiveAgentType === 'gemini')) {
      ...
      try {
        ...
        const forcedProvider = isObsidianHivemindAgentKey(selectedAgentKey) ? 'obsidian' : undefined;
        const initialMessage = {
          input,
          files: files.length > 0 ? files : undefined,
          provider: forcedProvider ?? (typeof currentModel?.id === 'string' ? currentModel.id : selectedAgent),
