...
import { StatBadge, OverviewTab, TasksTab, MessagesTab, AnalyticsTab } from './components';
...
const TeamDetailPage: React.FC = () => {
  ...
  const refresh = async () => {
    ...
    setLoading(true);
    try {
      const [nextTeam, nextTeammates, nextTasks, nextMessages, nextStats, nextCost] = await Promise.all([agentTeamsApi.getTeam(teamId), agentTeamsApi.listTeammates(teamId), agentTeamsApi.listTasks(teamId), agentTeamsApi.getMessages(teamId), agentTeamsApi.getTeamStats(teamId), agentTeamsApi.getCostAnalysis(teamId)]);
      setTeam(nextTeam);
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className='px-5 pt-2'>
              <TabsTrigger value='overview'>
                <Typography variant='body2' bold={activeTab === 'overview'}>
                  Overview
                </Typography>
              </TabsTrigger>
              ...
              <TabsTrigger value='messages'>
                <Typography variant='body2' bold={activeTab === 'messages'}>
                  Messages
                </Typography>
              </TabsTrigger>
              <TabsTrigger value='analytics'>
                <Typography variant='body2' bold={activeTab === 'analytics'}>
                  Analytics
                </Typography>
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value='overview' className='p-0'>
              <OverviewTab team={team} stats={stats} onRefresh={refresh} onQuickCreateTask={handleQuickCreateTask} />
            </TabsContent>
                ...
                <Card className='mb-6'>
                  ...
                  <CardContent>
                    <div className='flex items-end gap-2'>
                      <Input placeholder='Name' value={teammateName} onChange={(e) => setTeammateName(e.target.value)} />
                      <Input placeholder='Role' value={teammateRole} onChange={(e) => setTeammateRole(e.target.value)} />
                      <Input placeholder='Provider' value={teammateProvider} onChange={(e) => setTeammateProvider(e.target.value)} />
                      <Input placeholder='Model' value={teammateModel} onChange={(e) => setTeammateModel(e.target.value)} />
