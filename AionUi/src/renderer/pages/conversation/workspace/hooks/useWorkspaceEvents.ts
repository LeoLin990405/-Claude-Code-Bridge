...
interface UseWorkspaceEventsOptions {
  ...
  eventPrefix: 'gemini' | 'acp' | 'codex' | 'hivemind' | 'openclaw-gateway';
...
export function useWorkspaceEvents(options: UseWorkspaceEventsOptions) {
  ...
  useEffect(() => {
    ...
    const handleHivemindResponse = (data: { type: string }) => {
      if (data.type === 'tool_group' || data.type === 'tool_call') {
        refreshWorkspace();
      }
    };
    ...
    const unsubscribeHivemind = ipcBridge.hivemindConversation.responseStream.on((message) => {
      if (message.conversation_id !== conversation_id) {
        return;
      }
      handleHivemindResponse(message);
    });
   ...
   */
  useAddEventListener(`${eventPrefix}.workspace.refresh` as any, () => refreshWorkspace(), [refreshWorkspace]);

  /**
   * 监听清空选中文件事件（发送消息后）
   * Listen to clear selected files event (after sending message)
   */
  useAddEventListener(`${eventPrefix}.selected.file.clear` as any, () => clearSelection(), [clearSelection]);
