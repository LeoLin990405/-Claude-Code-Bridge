...
export const createHivemindAgent = async (options: ICreateConversationParams): Promise<TChatConversation> => {
  const { extra } = options;
  const { workspace, customWorkspace } = await buildWorkspaceWidthFiles(`hivemind-temp-${Date.now()}`, extra.workspace, extra.defaultFiles, extra.customWorkspace);
  ...
  return {
    ...
    extra: {
      ...
      timeoutS: extra.timeoutS || 300,
      streaming: extra.streaming !== false,
      agent: extra.agent || null,
      cacheBypass: extra.cacheBypass === true,
