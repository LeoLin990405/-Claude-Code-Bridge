...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'hivemind');
};
...
/**
 * Get data path, using CLI-safe symlink (~/.hivemind) on macOS.
 * 获取数据目录路径，macOS 上使用 ~/.hivemind 符号链接。
 */
export const getDataPath = (): string => {
  const rootPath = app.getPath('userData');
  const dataPath = path.join(rootPath, 'hivemind');
  return ensureCliSafeSymlink(dataPath, '.hivemind');
};

/**
 * Get config path, using CLI-safe symlink (~/.hivemind-config) on macOS.
 * 获取配置目录路径，macOS 上使用 ~/.hivemind-config 符号链接。
 */
export const getConfigPath = (): string => {
  const rootPath = app.getPath('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, '.hivemind-config');
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[HiveMind] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (dir: string, files?: string[], skipCleanup = false): Promise<string[]> => {
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[HiveMind] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[HiveMind] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[HiveMind] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
