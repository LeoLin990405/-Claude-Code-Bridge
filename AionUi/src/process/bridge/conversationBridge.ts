...
import type HivemindAgentManager from '../task/HivemindAgentManager';
...
export function initConversationBridge(): void {
  ...
  ipcBridge.conversation.stop.provider(async ({ conversation_id }) => {
    ...
    if (task.type !== 'gemini' && task.type !== 'acp' && task.type !== 'codex' && task.type !== 'openclaw-gateway' && task.type !== 'hivemind') {
  ...
  ipcBridge.conversation.sendMessage.provider(async ({ conversation_id, files, ...other }) => {
    ...
    let task: GeminiAgentManager | AcpAgentManager | CodexAgentManager | OpenClawAgentManager | HivemindAgentManager | undefined;
    try {
      task = (await WorkerManage.getTaskByIdRollbackBuild(conversation_id)) as GeminiAgentManager | AcpAgentManager | CodexAgentManager | OpenClawAgentManager | HivemindAgentManager | undefined;
    } catch (err) {
